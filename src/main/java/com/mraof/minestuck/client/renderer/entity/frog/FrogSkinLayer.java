...
public class FrogSkinLayer extends RenderLayer<FrogEntity, FrogModel<FrogEntity>>
{
	...
	{
		if (!frog.isInvisible())
			coloredCutoutModelCopyLayerRender(this.getParentModel(), this.frogModel, Minestuck.id("textures/entity/frog/skin_" + frog.getFrogVariant().getSerializedName() + ".png"),
					poseStack, bufferIn, packedLightIn, frog, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch, partialTicks,
					frog.getFrogVariant() == FrogEntity.FrogVariants.DEFAULT ? frog.getSkinColor() : 0xFFFFFFFF);
