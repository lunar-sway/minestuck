...
public class GunItem extends Item
{
	...
	public ActionResult<ItemStack> onItemRightClick(World world, PlayerEntity player, Hand hand)
	{
		...
		if(!doNotProceedIfCrouch(player))
		{
			...
			if(foundItem)
			{
				...
				itemStackIn.damageItem(1, player, playerEntity -> playerEntity.sendBreakAnimation(hand));
	...
	void gunShoot(World world, ServerPlayerEntity player, ItemStack bulletStack)
	{
		if(bulletStack.getItem() instanceof BulletItem)
		{
			...
			//bulletType = bulletItem.getType();
			
			if(sound == null) //default parameters
			{
			...
			int travelDistance = distance + bulletItem.getDistance() + player.getRNG().nextInt(((int) distance / 10)); //combines bullet and gun mods plus a randomness factor
	...
	{
		if(type == null) //TODO not sure if this is where it breaks, but specifically when a Type-less bullet hits entities there is no particle effects(meaning it works fine with any bullet if nothing is hit)
