		...
		if(entityLiving instanceof Player)
		{
			...
			if(!playerIn.isCreative() && level.getBlockState(pos).getBlock() == Blocks.TALL_GRASS)
			{
				...
				if(!level.isClientSide)
				{
					...
					{
						ItemEntity item = new ItemEntity(level, pos.getX(), pos.getY() + 0.5, pos.getZ(), new ItemStack(MSItems.GOLDEN_GRASSHOPPER, 1));
					...
					{
						ItemEntity item = new ItemEntity(level, pos.getX(), pos.getY() + 0.5, pos.getZ(), new ItemStack(MSItems.GRASSHOPPER, 1));
						level.addFreshEntity(item);
						playerIn.getMainHandItem().hurtAndBreak(1, playerIn, PlayerIn -> playerIn.broadcastBreakEvent(InteractionHand.MAIN_HAND));
