...
public class CloseSburbConnectionPacket implements MSPacket.PlayToServer
{
	...
	public void execute(ServerPlayer player)
	{
		ComputerBlockEntity.forNetworkIfPresent(player, pos, computer -> {
			if(isClient)
				SkaianetComputerInteractions.get(player.server).closeClientConnection(computer);
			else SkaianetComputerInteractions.get(player.server).closeServerConnection(computer);
		});
	}
}