...
import com.mraof.minestuck.world.gen.DerseGenSettings;
...
public class DerseDimension extends Dimension
{
	public DerseDimension(World worldIn, DimensionType typeIn)
	{
	...
	{
		DerseGenSettings settings = MSWorldGenTypes.DERSE.createSettings();
		settings.setDefaultBlock(Blocks.STONE.getDefaultState());
		settings.setDefaultFluid(Blocks.AIR.getDefaultState());
		return MSWorldGenTypes.DERSE.create(this.world, BiomeProviderType.FIXED.create(BiomeProviderType.FIXED.createSettings().setBiome(MSBiomes.DERSE)), settings);
	}
