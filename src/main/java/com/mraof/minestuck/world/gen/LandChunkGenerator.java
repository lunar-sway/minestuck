...
public class LandChunkGenerator extends AbstractChunkGenerator
{
	...
	private final LandBiomeHolder biomes;
	private final GristTypeLayer anyGristLayer, commonGristLayer, uncommonGristLayer;
	
	public LandChunkGenerator(long seed, LandTypePair landTypes)
	{
		this(seed, LandProperties.create(landTypes), new LandGenSettings(landTypes));
	}
	
	private LandChunkGenerator(long seed, LandProperties properties, LandGenSettings genSettings)
	{
		this(seed, new LandBiomeHolder(genSettings, properties), properties, genSettings);
	}
	
	private LandChunkGenerator(long seed, LandBiomeHolder biomes, LandProperties properties, LandGenSettings genSettings)
	{
		super(new LandBiomeProvider(seed, biomes, genSettings),
				new LandBiomeProvider(seed, properties.biomes, genSettings), seed, genSettings.createDimensionSettings());
	...
	{
		return new LandChunkGenerator(seed, landTypes);
	}
