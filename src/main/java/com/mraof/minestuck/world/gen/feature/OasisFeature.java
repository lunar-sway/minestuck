	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context)
	{
		...
		for (int x = 1; x < 15; x++)
		{
			for (int z = 1; z < 15; z++)
			{
				...
				{
					BlockPos topPos = level.getHeightmapPos(Heightmap.Types.WORLD_SURFACE_WG, pos.offset(x - 8, 0, z - 8)).below();
		...
		for (int x = 0; x < 16; x++)
		{
			for (int z = 0; z < 16; z++)
			{
				for (int y = 0; y < 4; y++)
				{
					...
					if (blocks[index])
						setBlock(level, pos.offset(x - 8, y - 3, z - 8), Blocks.WATER.defaultBlockState());
					else if (!blocks[index] && hasBlock1(blocks, x, y, z, false))
						setBlock(level, pos.offset(x - 8, y - 3, z - 8), Blocks.DIRT.defaultBlockState());
				}
				...
				if (!blocks[((x * 16) + z) * 4 + 3] && hasBlock2(blocks, x, 3, z))
				{
					...
					for (int y = 0; y < 4; y++)
						setBlock(level, pos.offset(x - 8, y + 1, z - 8), Blocks.AIR.defaultBlockState());
			}
		}
		...
		if(treePos != null)	//This should never not be the case
		{
			...
			for (int x = -4; x <= 4; x++)
			{
				...
				{
					int lowerY = 1;
					if (rand.nextDouble() < Math.sqrt(BlockPos.ZERO.distToLowCornerSqr(x, 0, z)) / 4)
						lowerY = 0;
					...
					for (int y = lowerY; y <= upperY; y++)
						setBlock(level, topPos.offset(x, y, z), leaves);
				}
			}
			setBlock(level, topPos.above(), log);
		}
		...
		if(rand.nextInt(25) == 0)
		{
			...
			if(chestPos != null)
			{
				StructureBlockUtil.placeLootChest(chestPos, level, null, Direction.Plane.HORIZONTAL.getRandomDirection(rand), MSLootTables.BASIC_MEDIUM_CHEST, rand);
			}
		}
