...
public class DerseTransportalizerTileEntity extends TileEntity //implements ITeleporter
{
	public GlobalPos destination = GlobalPos.of(DimensionType.OVERWORLD, new BlockPos(0, -1, 0));

	public DerseTransportalizerTileEntity()
	{
		super(MSTileEntityTypes.DERSE_TRANSPORTALIZER);
	}
	...
	public void setWorld(World worldIn)
	{
		...
		if(destination.getDimension() == worldIn.getDimension().getType())
			destination = GlobalPos.of(worldIn.getDimension().getType() == MSDimensions.derseDimension ? DimensionType.OVERWORLD : MSDimensions.derseDimension, destination.getPos());
	}
