...
public final class ConsortDialogue
{
	...
	private static void consortDialogues(SelectableDialogueProvider provider, DialogueLangHelper l)
	{
		provider.getLookupProvider().thenCompose(providerIt ->
		{
			...
			provider.addRandomlySelectable("best_village", defaultWeight(all(isAnyEntityType(SALAMANDER, IGUANA, NAKAGATOR), isInStructure(structures.getOrThrow(MSStructures.ConsortVillage.KEY)))),
					new NodeBuilder(l.defaultKeyMsg("This village? Yeah, this is the best village around. You haven't been to a village until you've been here.")));
	...
	private static void terrainFaygoDialogue(SelectableDialogueProvider provider, DialogueLangHelper l, DeferredItem<Item> drink, String capitalizedDrinkName, DeferredItem<Item> scalemateItem, String scalemateCapitalizedName, Condition terrain)
	{
		...
		{
			var faygoConditionText = l.subText("condition", "Must have the correct faygo");
