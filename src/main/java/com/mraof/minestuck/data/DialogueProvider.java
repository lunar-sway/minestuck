...
public abstract class DialogueProvider implements DataProvider
{
	...
	@SafeVarargs
	public static Condition isEntityType(RegistryObject<EntityType<ConsortEntity>>... entityType)
	{
		return isEntityType(Arrays.stream(entityType).map(RegistryObject::get).toArray(EntityType<?>[]::new));
	}
