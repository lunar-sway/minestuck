	...
	public void generate(BiConsumer<ResourceKey<LootTable>, LootTable.Builder> lootProcessor)
	{
		...
		lootProcessor.accept(MSLootTables.LOTUS_FLOWER_DEFAULT, LootTable.lootTable()
				...
				.withPool(LootPool.lootPool().name("code").setRolls(ConstantValue.exactly(1))
						.add(LootItem.lootTableItem(MSItems.SBURB_CODE.get()).setWeight(10).setQuality(3))/*.apply(SetNbtFunction.setTag(IncompleteSburbCodeItem.giveParadoxInfo())) TODO for when dweb is done with componentizing sburb code*/)
		);
	}
}