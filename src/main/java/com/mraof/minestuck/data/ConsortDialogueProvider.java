...
public final class ConsortDialogueProvider extends SelectableDialogueProvider
{
	...
	{
		ConsortDialogueProvider provider = new ConsortDialogueProvider(output);
		...
		provider.addDialogue(new DialogueLangHelper(enUsLanguageProvider));
		
		return provider;
	}
	
	private ConsortDialogueProvider(PackOutput output)
	{
		super(Minestuck.MOD_ID, RandomlySelectableDialogue.DialogueCategory.CONSORT, output);
	}
	
	private void addDialogue(DialogueLangHelper l)
	{
		consortDialogues(l);
		testDialogues(l);
	}
	
	private void consortDialogues(DialogueLangHelper l)
	{
		//Wind
		...
		add(rabbitFood2, new NodeSelectorBuilder()
				...
				.defaultNode(new NodeBuilder(l.subMsg("a", "There's not really much food to be found in this desolate place."))
						.addResponse(new ResponseBuilder(ARROW).condition(isInTerrainLand(MSTags.TerrainLandTypes.SAND))
								.nextDialogue(add("rabbit.food.3", new NodeBuilder(l.defaultKeyMsg("Except maybe cacti, but would rabbits eat something that prickly?")))))));
		...
		addRandomlySelectable("oil_buckets.1", defaultWeight(all(isInTitle(BUCKETS), isInTerrain(SHADE))),
				new NodeBuilder(l.defaultKeyMsg("Did you know that the buckets sometimes hold something other than oil?"))
						.next(add("oil_buckets.2", new NodeBuilder(l.defaultKeyMsg("In some cases, they even contain something drinkable!")))));
		...
		addRandomlySelectable("height_fear", defaultWeight(all(any(isInTitle(TOWERS), isInTitle(WIND)), isAnyEntityType(TURTLE))), new NodeSelectorBuilder()
				...
				.node(isInTitle(TOWERS), new NodeBuilder(l.subMsg("towers.1", "I'd climb up one of those towers and look at the view, but I am scared of heights."))
						.next(add("height_fear.towers.2", new NodeBuilder(l.defaultKeyMsg("I mean, what if I slipped and fell off the stairs?")))))
				.defaultNode(new NodeBuilder(l.subMsg("rock.1", "I'd climb up one of those rocks and look at the view, but I am scared of heights."))
						.next(add("height_fear.rock.2", new NodeBuilder(l.defaultKeyMsg("I mean what if I fell down and landed on my back?"))))));
		...
		addRandomlySelectable("mushroom_pizza", defaultWeight(isInTerrain(SHADE)),
				new NodeBuilder(l.defaultKeyMsg("Do you put glow mushrooms on your pizza or leave them off?"))
						.addResponse(new ResponseBuilder(l.subMsg("reply_on", "I put them on!"))
								.nextDialogue(add("mushroom_pizza.on", new NodeBuilder(l.defaultKeyMsg("Good! I was afraid I'd have to kill you!")))))
						.addResponse(new ResponseBuilder(l.subMsg("reply_off", "I leave them off!"))
								.nextDialogue(add("mushroom_pizza.off", new NodeBuilder(l.defaultKeyMsg("You are a despicable person."))))));
		...
		addRandomlySelectable("camel", defaultWeight(isInTerrainLand(MSTags.TerrainLandTypes.SAND)), new NodeBuilder(l.defaultKeyMsg("Want to buy a used camel? Only 2000 boondollars."))
				.addResponse(new ResponseBuilder(l.subMsg("yes", "Why not? Seems like a good price for a camel!"))
						.nextDialogue(add("camel/no_camel", new NodeBuilder(l.defaultKeyMsg("Hahaha! Sucker! I have no camel! Cya later! 8)")))))
				.addResponse(new ResponseBuilder(l.subMsg("no", "Of course not! You know better!"))
						.nextDialogue(add("camel/dancing_camel", new NodeBuilder(l.defaultKeyMsg("Are you sure? Too bad! The camel knew how to dance, too!"))))));
		...
		addRandomlySelectable("bloodberries.1", defaultWeight(isInTerrain(FLORA)),
				new NodeBuilder(l.defaultKeyMsg("The strawberries here grow big and red thanks to all the blood in the water supply! The flowers thrive, too!"))
						.next(add("bloodberries.2", new NodeBuilder(l.defaultKeyMsg("Strawberry juice is the only thing safe to drink here. If I have any more, I'll scream. Please save us.")))));
	...
	private void testDialogues(DialogueLangHelper l)
	{
		var test1 = dialogueId("test1");
		var test2 = dialogueId("test2");
		
		add(test1, new NodeBuilder(l.defaultKeyMsg("Press §eSHIFT§r for more info"))
		...
		addRandomlySelectable("test_arguments", defaultWeight(Condition.AlwaysTrue.INSTANCE), new NodeBuilder(l.defaultKeyMsg("Player name land: %s", DialogueMessage.Argument.PLAYER_NAME_LAND))
