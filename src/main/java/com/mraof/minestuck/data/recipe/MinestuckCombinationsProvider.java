...
public class MinestuckCombinationsProvider extends RecipeProvider
{
	...
	protected void registerRecipes(Consumer<IFinishedRecipe> consumer)
	{
		...
		CombinationRecipeBuilder.of(MSItems.spikedGlove).input(Blocks.IRON_BARS).and().input(MSItems.fancyGlove).build(consumer);
		CombinationRecipeBuilder.of(MSItems.cobbleBasher).input(Blocks.COBBLESTONE).and().input(MSItems.fancyGlove).build(consumer);
		CombinationRecipeBuilder.of(MSItems.pogoFist).input(Items.SLIME_BALL).or().input(MSItems.fancyGlove).build(consumer);
		CombinationRecipeBuilder.of(MSItems.fluoriteGauntlet).input(MSItems.FLUORITE_OCTET).or().input(MSItems.cobbleBasher).build(consumer);
		
		//bows
		CombinationRecipeBuilder.of(MSItems.natureBow).input(Items.BOW).and().input(Blocks.VINE).build(consumer);
		CombinationRecipeBuilder.of(MSItems.infernoShot).input(Items.BOW).and().input(Items.BLAZE_POWDER).build(consumer);
		CombinationRecipeBuilder.of(MSItems.energyBow).input(Items.BOW).and().input(MSItems.ENERGY_CORE).build(consumer);
