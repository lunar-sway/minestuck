...
public class MinestuckRecipeProvider extends RecipeProvider
{
	...
	protected void registerRecipes(Consumer<IFinishedRecipe> recipeBuilder)
	{
		...
		ShapedRecipeBuilder.shapedRecipe(MSItems.FLINTLOCK_PISTOL, 1).key('i', Items.IRON_INGOT).key('f', Items.FLINT).key('g', Items.GOLD_INGOT).key('w', ItemTags.PLANKS).patternLine("iif").patternLine(" gi").patternLine(" iw").addCriterion("has_iron_ingot", hasItem(Items.IRON_INGOT)).build(recipeBuilder);
